		...
		function displayRecentOrders() {
			...
			if (recentOrders.length === 0) {
				container.innerHTML = '<p>No orders yet. <a href="products.php">Start shopping!</a></p>';
				return;
			}
			...
			if (userOrders.length === 0) {
				container.innerHTML = `
					<div style="text-align: center; padding: 50px;">
						...
						<a href="products.php" class="primary-btn">Start Shopping</a>
					</div>
				`;
				return;
			}
		...
		async function loadUserWishlist() {
			try {
				const response = await fetch(`${API_BASE_URL}/wishlist`, { credentials: 'same-origin', headers: { 'Accept': 'application/json' } });
				if (response.ok) {
					const data = await response.json();
					userWishlist = Array.isArray(data) ? data : (data.items || data.wishlist || data);
				} else {
					userWishlist = [];
				}
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			document.getElementById('searchForm')?.addEventListener('submit', function(e) {
				...
				if (query.trim()) {
					window.location.href = `products.php?search=${encodeURIComponent(query)}&category=${category}`;
				}
			});
		});
